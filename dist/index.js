...
class WindowsManager {
    ...
    constructor(options = {}) {
        ...
        this.newWindowListeners = [];
        this.removeWindowListeners = [];
        this.windowUpdateListeners = [];
        this.activeWindowChangeListeners = [];
        ...
        this.handleWindowRectChange = () => {
            ...
            const changed = rect.screenX !== this.lastWindowRect.screenX ||
        ...
        if (options.onMessage)
            this.addMessageListener(options.onMessage);
        if (options.onNewWindow)
            this.addNewWindowListener(options.onNewWindow);
        ...
        if (options.onMetaChange)
            this.addMetaChangeListener(options.onMetaChange);
    ...
    removeNewWindowListener(listener) {
        this.newWindowListeners = this.newWindowListeners.filter((l) => l !== listener);
    }
    /**
    ...
    removeRemoveWindowListener(listener) {
        this.removeWindowListeners = this.removeWindowListeners.filter((l) => l !== listener);
    }
    /**
    ...
    removeWindowUpdateListener(listener) {
        this.windowUpdateListeners = this.windowUpdateListeners.filter((l) => l !== listener);
    }
    /**
    ...
    removeActiveWindowChangeListener(listener) {
        this.activeWindowChangeListeners = this.activeWindowChangeListeners.filter((l) => l !== listener);
    }
    /**
    ...
    removeMetaChangeListener(listener) {
        this.metaChangeListeners = this.metaChangeListeners.filter((l) => l !== listener);
    }
     ...
     */
    startMovePolling() {
        this.movePollInterval = window.setInterval(this.handleWindowRectChange, 300);
    }
    /**
    ...
    initCurrentWindow() {
        ...
        const rect = this.getWindowRect();
        const newWindow = { id: generateWindowId(), ...rect };
